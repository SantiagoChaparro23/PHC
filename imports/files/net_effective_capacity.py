...
class Parser:
    ...
    def parser(self):
        ...
        for row in self.rows:
            ...
            # name = row[1]
            id_resource = Resource.objects.filter(name = row[1]).values_list('id').first()
            if id_resource:
                ...
                if re.search('20[0-9][0-9]-[0-9][0-9]-[0-9][0-9]', row[0]):
                    ...
                    query = '''
                    INSERT INTO "imports_neteffectivecapacity" ( "date", "net_effective_capacity", "resource_id") VALUES
                    ({}, {});
                    '''.format(data, id_resource)
